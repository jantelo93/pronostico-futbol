...
ALIAS = {
 ...
 'aek athens':'aek athen','aek athina':'aek athen','aek':'aek athen','lask linz':'lask',
...
def traer_fixtures(token, equipos):
    fx, planteles = {}, {}
    if not token:
        print("  Sin token: no se descargan fixtures.")
        return fx, planteles
    ...
    for codigo, comp in COMPETENCIAS.items():
        ...
        fx[comp] = prox[:24]
        print(f"  {NOMBRES[comp]}: {len(fx[comp])} partidos pendientes, {len(planteles[comp])} equipos")
...
def main():
    ...
    print("3. Calculando Elo", flush=True)
    elo = calcular_elo(M)
    print("4. Ajustando ataque y defensa (nivel estructural)", flush=True)
    PL = ajustar(M, equipos, comps, 540.0, 0.02)
    print("5. Ajustando forma reciente", flush=True)
    ...
    print("6. Calculando momento", flush=True)
    mom = calcular_momento(M, equipos, attL, defL, cL, hL)
    print("7. Descargando partidos pendientes", flush=True)
    fx, planteles = traer_fixtures(os.environ.get("FOOTBALL_DATA_TOKEN", "").strip(), equipos)
    if not planteles:
